...
- `source/css/_tailwind.css` — core Tailwind CSS v4 source, theme tokens, core component styles, and custom utilities; compiled to `style.min.css`. The title font (`Yuji Syuku`) shares the critical `display=swap` request with the body families in `google_font_urls` and is preloaded in `head.njk`, so the title reliably loads and ends up in the brand font (a brief swap is possible on cold caches; `display=optional` was tried but left the title unloaded until cached). If you change the title font, update the critical family list in `google_font_urls`.
