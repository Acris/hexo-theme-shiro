...
- Home cards: `post_card_view_models` applies `excerpt_for_card` policy to the full list — no manual excerpt + fallback off → empty body + read-more (never full post HTML). The first rendered non-decorative image defaults to eager; later images default to lazy without overriding authored `loading` attributes. Reading templates apply the eager default after TOC rendering; the image filter leaves the first content-image policy deferred.
