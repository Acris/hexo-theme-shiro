...
- `layout/_partial/common/` — shared shell partials: `head.njk` (document head and SEO meta: description, Open Graph + `article:*`/`og:locale`/`og:image:alt`, Twitter Card, canonical, paginated `rel=prev`/`rel=next`, and schema.org JSON-LD emitted via the `structured_data` helper), `header.njk`, `footer.njk`, `pagination.njk`, `empty.njk`.
- `layout/_partial/components/` — page components: `article.njk`, `post-card.njk`, `toc.njk`. `article.njk` wraps the rendered post/page body in the sole `data-pagefind-body` region (everything else — header, nav, TOC, comments, footer — is `data-pagefind-ignore`) and tags the title `<h1>` with `data-pagefind-meta="title"` so Pagefind always uses the article title for the search-result title instead of relying on the first `<h1>` it happens to find; keep that attribute on the title when editing the header.
