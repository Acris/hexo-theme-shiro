...
| `source/js/_src/`          | Client sources → `source/js/*.min.js` (Hexo ignores `_src` via underscore prefix). Runtime: `runtime/*.js` sorted concat → `runtime.min.js` (loaded before feature bootstraps).                                 |
...
- Optional `security.csp_nonce` is a **static** theme config value (not per-request). Emitted on theme `<script>` tags via `csp_nonce_attr`, injected once as `window.__shiro.cspNonce` in head-theme, applied by `runtime.min.js` (from `source/js/_src/runtime/*`) to dynamic scripts. Real CSP nonce security needs host/edge injection of the same value. Optional CDN SRI via `sri_attrs` / `lightGallery.*_integrity` / `mathjax.integrity` (empty = no attributes).
...
- Client runtime source is split under `source/js/_src/runtime/*.js` (explicit `RUNTIME_PARTS` manifest in `tools/build-assets.js` → concat → `runtime.min.js`). Build fails on missing/extra parts or broken single-IIFE shape. Do not reintroduce a monolith `runtime.js`. Nested parts share one IIFE scope.
- Lazy client features use bootstrap + body scripts (`*-bootstrap.js` + feature file). Canonical loader: `runtime.loadBootstrapScript` / `runtime.createFeatureLoader` / `runtime.bindIntentWarm` with a short stable `id` + optional `scheduleIdleWarm`. Do not invent a parallel path. `createFeatureLoader` resolves only after `runtime.featureReady(id)` (script onload alone is not enough); early exits call `runtime.featureAbort(id)`. `featureAbort` / ready-timeout are **permanent** for that loader (no 8s retry hang); network fetch failures remain retryable. `load()` always fulfills after `onError` (call sites need no `.catch`; use `onError` for failure). `loadAsset` dedupes concurrent loads via an in-flight Map.
- Standard lazy open/warm handoff (currently LightGallery; reuse for similar bootstraps): `runtime.dispatchLiveOrStash` / `dispatchLiveOrWarm` — live API if present, else stash + load; live open `false` → navigate. Do not invent a parallel handoff.
- Client config lives on `window.__shiro` only (bare keys: `clipboardScript`, `cspNonce`, `commentsConfig`, `lightgallery`, …). Read via `runtime.get(...)` only — do not hang `get` on the bag root. Flat `window.__shiroRuntime` is the runtime API alias. LightGallery: **bootstrap owns document capture clicks for the page life** (hardFail only unbinds); feature installs `lightGalleryOpen` / `Warm` + CDN and does **not** register click. While loading, **first** autoOpen wins (later clicks ignored until ready). Hard-fail `navigateFromImage`s pending autoOpen. Shared: `safeNavigate` / `navigateFromImage` / `isModifiedClick`.
...
- Dark mode does **not** invert the Tailwind slate scale. Prefer semantic tokens over `text-slate-*`. UI idle / secondary chrome text uses **`text-chrome`** (not `text-muted`) for dark AA — layouts, component chrome, toc/code captions, search secondary. Token `--color-text-muted` remains in the design system but new UI should not prefer it over chrome.
- Comments boot: stub enqueue in `comments/bootstrap.njk` during parse (`__shiro.commentsReadyQueue`); `comments-bootstrap.js` (after `runtime.min.js`) installs helpers and drains the queue. Missing runtime aborts hard. Providers fall back to immediate load if `onNearViewport` is missing.
