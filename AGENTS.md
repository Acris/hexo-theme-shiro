...
- Client globals: dual-write into `window.__shiro` (bag) and legacy flat names (`window.__shiroRuntime`, `window.__lightgalleryJs`, …). New code should prefer `window.__shiro.runtime` / bag keys; do not add more top-level `window.__*` without also setting `__shiro`.
