...
- `source/css/_src/lightgallery.css` / `source/css/lightgallery.min.css` — optional plain CSS source and minified output for LightGallery theme overrides; loaded when the lightbox is warmed on the first image intent (hover/press/focus) or opened. `head.njk` also adds a `cdn.jsdelivr.net` preconnect on image pages so the warmed CDN assets download instantly.
