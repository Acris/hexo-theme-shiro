...
- Home/tag/category code gates analyze `excerpt_for_card` output, not hidden full post bodies; manual excerpts and fallback truncation therefore stay in sync with rendered cards.
...
- Collapsed mobile-menu / inline-TOC content must be `inert` so hidden links leave the tab order. JS-only header controls and disclosure buttons stay hidden until their client handler is ready. Mobile-menu permanent load failure restores the visible no-JS fallback; inline TOC is expanded without JS and collapses only after its deferred client initializes.
