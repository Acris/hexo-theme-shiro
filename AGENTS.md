...
- Comments readiness: prefer `gates.shiroComments` (same pure `resolveCommentsState` as optional `comments_state` helper). Containers: `comments/index.njk`; scripts: `comments/foot.njk` after deferred `runtime.min.js`. Client config: `comments_client_config` → `window.__shiroCommentsConfig`. Boot: sync queue stub in `comments/bootstrap.njk` → defer `comments-bootstrap.min.js` → defer `comments-giscus.min.js` or `comments-disqus.min.js` (providers call `__shiroWhenCommentsReady`).
...
- Client globals: dual-write into `window.__shiro` (bag) and legacy flat names (`window.__shiroRuntime`, `window.__lightgalleryJs`, …). Prefer `window.__shiro.runtime` and `runtime.get('clipboardScript')` (also tries bare + `__` keys). Do not add more top-level `window.__*` without dual-writing the bag (and a bare key when the flat name is `__foo`).
