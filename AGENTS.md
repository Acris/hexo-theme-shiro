...
- Client config lives on `window.__shiro` only (bare keys: `clipboardScript`, `cspNonce`, `commentsConfig`, …). Read via `runtime.get('clipboardScript')`. Flat `window.__shiroRuntime` remains the runtime API alias. LightGallery bootstrap hard-fails by unbinding click capture and navigating the image/href.
