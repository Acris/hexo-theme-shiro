...
- Current top-level config keys: `site`, `menu`, `excerpt`, `toc`, `lightGallery`, `mathjax`, `dark_mode`, `progress_bar`, `back_to_top`, `comments` (`disqus` / `giscus`), `analytics` (`google`), `search` (Pagefind). Renaming or removing any of them is a breaking change.
