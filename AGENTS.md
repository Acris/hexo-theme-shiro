...
Shiro (白) is a clean, minimalist, multilingual Hexo theme built with Nunjucks templates and Tailwind CSS v4. It focuses on typography, whitespace, performance, dark mode, table of contents, optional MathJax rendering, static search, comments, analytics, and minimal client-side JavaScript for static Hexo output.

## Repository layout

- `layout/` — Nunjucks templates for Hexo pages.
- `layout/_layout.njk` — base HTML layout, shared page shell, conditional script loading, and global i18n data injection. It gates MathJax by post/page predicates plus explicit page front matter (`mathjax: true`) and only sets `mathjax_src` when needed; `head.njk` then emits the inline `window.MathJax` config plus deferred CDN script from `<head>` after the first-paint-critical theme/font hints but before blocking stylesheets can delay discovery. `processEscapes` is always enabled so escaped dollar signs such as `\$5` stay prose. It also always renders the font-load preloader overlay (`#shiroPreloader`) as the first `<body>` child, showing an opaque themed veil with a minimal vermilion dot-ripple mark; `preloader.min.js` — loaded with `defer` from `head.njk` so its fetch starts during head parse — fades it out once the title font loads. The same-tab skip relies on `head.njk`'s synchronous inline script reading `sessionStorage` to add `shiro-preloader-done` before first paint; that inline script and `preloader.min.js` both carry `data-cfasync="false"` so Cloudflare Rocket Loader does not defer them past first paint (otherwise the overlay re-shows on every reload).
...
- `scripts/helpers.js` — custom Hexo helpers (`should_render_toc`, `build_toc`, `js_value`, `url_query`, `safe_url_for`, `safe_resource_url_for`, `link_target`, `is_blank_target`, `archive_url`, `google_font_urls`, `page_has_code`, `page_looks_long`, `versioned_url`, `has_images`, `first_image`, `excerpt_for`, `clean_description`, `copyright_year`, `build_page_title`, `seal_path_d`, `og_image`, `og_image_size`, `og_locale`, `structured_data`) and the dynamic `favicon.svg` generator (`favicon_svg`). Treat this list as authoritative — check `scripts/helpers.js` before adding new helpers or duplicating logic. TOC, image, code, long-page, description, and excerpt helpers share page-level cached analysis; pass the full `page` object to these helpers instead of `page.content` so the cache is reused. A `before_post_render` filter protects renderable Markdown math segments on pages with `mathjax: true` before the Markdown renderer can consume TeX punctuation escapes like `\[` or `\!`; the paired `after_post_render` filter restores those segments, restores the excerpt and `more` placeholders (so feed/sitemap plugins reading `post.more` never leak a `@@SHIRO_MATH_...@@` token), and leaves final typesetting to browser-side MathJax. Each math placeholder is keyed by a hash of its own segment (not the whole document) so heading anchor IDs stay stable across unrelated edits, and bare `\begin{env}...\end{env}` detection is limited to the `MATH_ENVIRONMENT_NAMES` whitelist of real math environments with a matching `\end`. The `seal` macro and the `favicon_svg` generator share the same SVG path constant (`SEAL_PATH_D`) exposed via `seal_path_d` — when tweaking the seal shape, edit only that constant. The SEO helpers `og_locale` (Open Graph locale from the page language), `og_image_size` (Open Graph image dimensions reused from the `width`/`height` that `scripts/images.js` injects into rendered content `<img>`, so social cards skip a second fetch), and `structured_data` (schema.org `BlogPosting`/`WebSite` JSON-LD nodes) are consumed by `head.njk`. The `archive_url` helper builds the yearly archive link from Hexo's `archive_dir` (not a hard-coded `archives/`) so the post `meta` macro stays correct on sites with a custom `archive_dir`.
...
- `source/css/_tailwind.css` — core Tailwind CSS v4 source, theme tokens, core component styles, and custom utilities; compiled to `style.min.css`. The title font (`Yuji Syuku`) shares the critical `display=swap` request with the body families in `google_font_urls` and is preloaded in `head.njk`, so the title reliably loads and ends up in the brand font (a brief swap is possible on cold caches; `display=optional` was tried but left the title unloaded until cached). If you change the title font, update the critical family list in `google_font_urls`. The non-critical `display=optional` families (`Cormorant Garamond`, plus `Fira Code` on code pages) load asynchronously in `head.njk` (`rel=preload`/`onload` swap) so they never block first paint or compete with the critical stylesheet. It also defines the `.shiro-preloader` overlay styles (opaque fog-toned veil with light/dark variants, minimal vermilion dot-ripple mark, and a pure-CSS failsafe that dismisses the overlay even without JS); these are plain selectors (not runtime Tailwind utilities) so the JS-toggled `.is-loaded` / `.shiro-preloader-done` classes are always compiled. It also keeps a plain `dialog.mjx-dialog` rule (in `@layer components`, so it overrides Preflight's `*{margin:0}`) that restores `margin: auto` to re-center MathJax v4's runtime-injected modal (e.g. the Explorer help box), which Preflight otherwise pins to the top-left corner; keep it a plain selector because `.mjx-dialog` never appears in scanned templates.
...
- `source/css/_src/search.css` / `source/css/search.min.css` — optional plain CSS source and minified output for the search modal and Pagefind UI accents; lazy-loaded when search is opened (and warmed ahead of it). `head.njk` adds lowest-priority `prefetch` hints for `search.min.js`/`search.min.css` and the Pagefind UI JS/CSS when `search.enabled`; `search-bootstrap.js` proactively warms on idle or first `#searchToggle` intent (hover/press/focus), gated on `navigator.connection` (skips `saveData`/`2g`), so the search box opens instantly even on touch devices with no hover. Warming loads the UI assets only — `search.js`'s `__shiroSearchWarm` does not instantiate `PagefindUI`, so the search index/WASM still load on first keystroke.
...
- `source/css/_src/lightgallery.css` / `source/css/lightgallery.min.css` — optional plain CSS source and minified output for LightGallery theme overrides; loaded when the lightbox is warmed (on first image intent — hover/press/focus, when the first image nears the viewport, or when opened). `head.njk` adds a `cdn.jsdelivr.net` preconnect plus lowest-priority `prefetch` hints for the LightGallery CDN JS/CSS and the same-origin wrapper — keep the prefetched `lightgallery@<version>` in sync with `source/js/_src/lightgallery.js`. The proactive viewport/idle warm lives in `lightgallery-bootstrap.js` (`IntersectionObserver` + `requestIdleCallback`), gated on `navigator.connection` (skips `saveData`/`2g`) so touch devices with no hover still open the lightbox instantly.
...
- `source/js/*.min.js` — generated browser script outputs: `theme-toggle`, `search`, `search-bootstrap`, `toc`, `progress`, `back-to-top`, `preloader`, `clipboard`, `clipboard-bootstrap`, `lightgallery`, `lightgallery-bootstrap`, `mobile-menu`, `mobile-menu-bootstrap`. `preloader.min.js` waits for the title font via `document.fonts` and fades out `#shiroPreloader` (with a connection-aware JS failsafe that fades sooner on `saveData`/`2g`, plus a warm-cache instant path). Some source files include build-time snippet markers such as `<shiro-asset-loader>` and must be consumed through `npm run build`, not served directly.
...
- Current top-level config keys: `site`, `menu`, `excerpt`, `toc`, `mathjax`, `dark_mode`, `progress_bar`, `back_to_top`, `comments` (`disqus` / `giscus`), `analytics` (`google`), `search` (Pagefind). Renaming or removing any of them is a breaking change.
...
- Gate each `<script>` tag in `layout/_layout.njk` by the narrowest applicable page predicates, feature toggles, and DOM needs (`is_home()` / `is_post()` / `is_page()`, `theme.toc.enabled`, `page.mathjax`, `theme.search.enabled`, rendered content checks, etc.) so unused pages stay JS-free.
