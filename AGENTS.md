...
- `source/js/*.min.js` — generated browser script outputs: `theme-toggle`, `search-bootstrap`, `toc`, `progress`, `back-to-top`, `preloader`, `clipboard`, `clipboard-bootstrap`, `lightgallery`, `lightgallery-bootstrap`, `mobile-menu`, `mobile-menu-bootstrap`. `preloader.min.js` waits for `document.fonts.ready` and fades out `#shiroPreloader` (with a bounded JS failsafe plus a warm-cache instant path). Some source files include build-time snippet markers such as `<shiro-asset-loader>` and must be consumed through `npm run build`, not served directly.
...
- Manually inspect generated pages when possible: home, post, page, archive, tag, category, dark mode, TOC, comments, search trigger/modal, code blocks, image lightbox, and responsive layout.
