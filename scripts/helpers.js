...
// Whitelisted math environments that may appear as a bare `\begin{env}...\end{env}`
// block (no surrounding `$$` / `\[`). Restricting detection to real math
// environments keeps prose such as `\begin{document}` or `\begin{itemize}` from
// being protected, then rendered as a MathJax "unknown environment" error.
const MATH_ENVIRONMENT_NAMES = [
    'align', 'alignat', 'aligned', 'alignedat', 'array',
    'Bmatrix', 'bmatrix', 'cases', 'CD', 'dcases', 'drcases',
    'eqnarray', 'equation', 'flalign', 'gather', 'gathered',
    'matrix', 'multline', 'pmatrix', 'rcases', 'smallmatrix',
    'split', 'subarray', 'Vmatrix', 'vmatrix'
];
// Longest names first so the alternation prefers e.g. `alignedat` over `aligned`
// over `align`; a trailing `*` (starred variant) is optional.
const MATH_ENVIRONMENT_NAME_PATTERN = '(?:'
    + MATH_ENVIRONMENT_NAMES.slice().sort((a, b) => b.length - a.length).join('|')
    + ')\\*?';
// Sticky opener used by the source scanner (scanMarkdownEnvironmentMath) to match
// a whitelisted `\begin{env}` exactly at a given index without slicing the source.
const MATH_ENVIRONMENT_OPENER_RE = new RegExp('\\\\begin\\s*\\{(' + MATH_ENVIRONMENT_NAME_PATTERN + ')\\}', 'y');
const SHIRO_MATH_PLACEHOLDERS = '__shiroMathPlaceholders';
...
function mathEnabledForPage(page) {
    return !!(page && page.mathjax === true);
}

function atMarkdownLineStart(source, index) {
...
function skipMarkdownFenceBlock(source, start) {
    if (!atMarkdownLineStart(source, start)) return start;
...
function skipMarkdownIndentedCodeBlock(source, start) {
    if (!atMarkdownLineStart(source, start) || !/^(?: {4}|\t)/.test(source.slice(start, start + 4))) {
        return start;
    }
...
// Whether the newline at `index` begins a blank line: only whitespace remains
// until the next line break or the end of input. Markdown inline constructs
// (code spans, `$...$` math) never span such a paragraph break.
...
function skipMarkdownCodeSpan(source, start) {
    ...
    // A code span never spans a blank line (CommonMark). If one appears before the
    // closing backticks, treat the run as literal so following math stays protected.
...
function scanMarkdownEnvironmentMath(source, start) {
    // Sticky match at `start` plus a lastIndex-driven closer avoid slicing the
    // source for every `\begin`, so pathological input stays linear rather than
    // quadratic. Only whitelisted environments (matching `\end`) are captured.
    MATH_ENVIRONMENT_OPENER_RE.lastIndex = start;
    const opener = MATH_ENVIRONMENT_OPENER_RE.exec(source);
    if (!opener) return '';

    const closeRe = new RegExp('\\\\end\\s*\\{' + escapeRegExp(opener[1]) + '\\}', 'g');
    closeRe.lastIndex = start + opener[0].length;
    const closer = closeRe.exec(source);
    if (!closer) return '';

    return source.slice(start, closer.index + closer[0].length);
}
...
function validSingleDollarOpener(source, start) {
    if (source[start] !== '$' || source[start + 1] === '$') return '';

    const previous = start > 0 ? source[start - 1] : '';
    const next = source[start + 1] || '';
    return !(escapedAt(source, start) || (previous && /[\w$]/.test(previous)) || !next || /[\s$]/.test(next));
}

function validSingleDollarCloser(source, index) {
    if (source[index] !== '$' || escapedAt(source, index)) return false;

    const previous = source[index - 1] || '';
    const next = source[index + 1] || '';
    return !/[\s$]/.test(previous) && next !== '$' && (!next || !/[\d\w]/.test(next));
}

function scanSingleDollarMath(source, start) {
    if (!validSingleDollarOpener(source, start)) return '';

    for (let cursor = start + 2; cursor < source.length; cursor += 1) {
        // Inline math never spans a blank line; stop before one so a stray `$` in
        // prose cannot merge paragraphs into a single hidden, mis-rendered block
        // (and so a lone `$` never scans to end of document).
        if (source[cursor] === '\n' && startsBlankLine(source, cursor)) return '';
        if (source[cursor] !== '$') continue;
        if (escapedAt(source, cursor)) continue;
        if (!validSingleDollarCloser(source, cursor)) return '';
...
function scanMarkdownMathAt(source, start) {
    let end;

    if (source.startsWith('\\[', start)) {
        end = findClosingDelimiter(source, start + 2, '\\]');
    ...
    if (source.startsWith('\\begin', start)) {
        return scanMarkdownEnvironmentMath(source, start);
    }

    return scanSingleDollarMath(source, start);
}

function mathSegmentToken(segment, salt) {
    return crypto
        .createHash('sha1')
        .update(salt ? segment + '\n' + salt : segment)
        .digest('hex')
...
function protectMarkdownMathContent(content) {
    const source = String(content || '');
    if (!source) return { content: source, placeholders: null };
    ...
    function placeholder(segment) {
        // Key each placeholder by a hash of the segment itself (not of the whole
        // document) so heading anchor IDs derived from the placeholder stay stable
        // when unrelated parts of the post change. Identical segments share a key
        // and restore to the same TeX; a hash collision with a different segment or
        // a literal clash in the source is broken with an incrementing salt.
        let salt = 0;
        let key;
        do {
            key = mathSegmentToken(segment, salt);
            salt += 1;
        } while (
            (segments[key] !== undefined && segments[key] !== segment)
            || source.includes('@@SHIRO_MATH_' + key + '@@')
        );
        segments[key] = segment;
        count += 1;
        return '@@SHIRO_MATH_' + key + '@@';
    }

    while (cursor < source.length) {
        const fenceEnd = skipMarkdownFenceBlock(source, cursor);
        if (fenceEnd > cursor) {
        ...
        const indentedCodeEnd = skipMarkdownIndentedCodeBlock(source, cursor);
        if (indentedCodeEnd > cursor) {
            result += source.slice(cursor, indentedCodeEnd);
            cursor = indentedCodeEnd;
            continue;
        }

        const codeSpanEnd = skipMarkdownCodeSpan(source, cursor);
        if (codeSpanEnd > cursor) {
            result += source.slice(cursor, codeSpanEnd);
            cursor = codeSpanEnd;
            continue;
        }

        const math = scanMarkdownMathAt(source, cursor);
        if (math) {
    ...
    return {
        content: count ? result : source,
        placeholders: count ? { segments } : null
    };
}

function restoreProtectedMathContent(content, placeholders) {
    const source = String(content || '');
    if (!placeholders || !placeholders.segments) return source;

    const segments = placeholders.segments;
    return source.replace(/@@SHIRO_MATH_([0-9a-f]{12})@@/g, (match, key) => {
...
// Markdown renderers commonly treat TeX punctuation escapes as Markdown escapes
// (`\[` -> `[`, `\!` -> `!`). Protect math before rendering so MathJax receives
// the author's original TeX, then restore it in after_post_render.
hexo.extend.filter.register('before_post_render', function (data) {
    if (!data || typeof data.content !== 'string' || !data.content) return data;

    if (!mathEnabledForPage(data)) return data;

    const protectedMath = protectMarkdownMathContent(data.content);
    if (!protectedMath.placeholders) return data;

    data.content = protectedMath.content;
    setHiddenPostValue(data, SHIRO_MATH_PLACEHOLDERS, protectedMath.placeholders);
    return data;
});

// Restore math placeholders after Markdown rendering. The MathJax script is
// loaded only when the page explicitly opts in with `mathjax: true`.
hexo.extend.filter.register('after_post_render', function (data) {
    if (!data || typeof data.content !== 'string' || !data.content) return data;

    if (!mathEnabledForPage(data)) return data;

    const placeholders = data[SHIRO_MATH_PLACEHOLDERS];
    if (!placeholders) return data;

    data.content = restoreProtectedMathContent(data.content, placeholders);
    // Hexo's excerpt filter splits rendered content into `excerpt` and `more`;
    // restore both so feed/sitemap plugins that read `post.more` never emit a
    // leftover `@@SHIRO_MATH_...@@` placeholder to readers.
    if (typeof data.excerpt === 'string' && data.excerpt) {
        data.excerpt = restoreProtectedMathContent(data.excerpt, placeholders);
    }
    if (typeof data.more === 'string' && data.more) {
        data.more = restoreProtectedMathContent(data.more, placeholders);
    }
    return data;
});
